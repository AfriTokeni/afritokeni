 ...
 */
export async function getPendingWithdrawals(agentPrincipal: string): Promise<WithdrawalTransaction[]> {
	const actor = await createWithdrawalActor();
	return await actor.get_pending_withdrawals(Principal.fromText(agentPrincipal));
}
 ...
 * QUERY (read-only)
 */
export async function getAgentWithdrawals(agentPrincipal: string): Promise<WithdrawalTransaction[]> {
	const actor = await createWithdrawalActor();
 ...
 */
export async function getAgentEarnings(agentPrincipal: string): Promise<AgentEarnings | null> {
	const actor = await createWithdrawalActor();
	const result = await actor.get_agent_earnings(Principal.fromText(agentPrincipal));
 ...
 * QUERY (read-only)
 */
export async function getUserWithdrawals(userPrincipal: string): Promise<WithdrawalTransaction[]> {
	const actor = await createWithdrawalActor();
 ...
 */
export async function getFeeSplit(): Promise<{ platformFeeBps: number; agentFeeBps: number }> {
	const actor = await createWithdrawalActor();
...
export function formatAgentEarnings(earnings: AgentEarnings) {
	return {
		...
		lastWithdrawalDate: earnings.last_withdrawal_date.length > 0 
