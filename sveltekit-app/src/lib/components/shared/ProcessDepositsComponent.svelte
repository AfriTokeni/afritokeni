<script lang="ts">
	...
	async function loadDepositRequests(isDemoMode: boolean, agentPrincipal: string | null) {
...
{#if showInstructions}
	<div class="bg-blue-50 border border-blue-200 rounded-2xl p-4 sm:p-5 md:p-6 mb-6">
		<div class="flex items-start justify-between mb-2 sm:mb-3">
			<h3 class="font-semibold text-blue-900 text-sm sm:text-base">Deposit Process:</h3>
			<button
		...
		<div class="mt-3 sm:mt-4 p-2.5 sm:p-3 bg-blue-100 rounded-lg">
			<p class="text-xs sm:text-sm text-blue-900 font-medium">
				⚠️ Always verify the deposit code before accepting cash. Rejected deposits cannot be
				reversed.
			</p>
		</div>
		{#if $demoMode}
...
<div class="bg-white rounded-2xl border border-gray-200 p-4 sm:p-5 md:p-6">
	...
	<div class="flex items-center justify-between mb-4">
		<h2 class="text-lg sm:text-xl font-semibold text-gray-900">Deposit Requests</h2>
		<button
	...
	<div class="space-y-3 sm:space-y-4">
		{#if loading}
			<div class="text-center py-8 sm:py-10 md:py-12">
				...
				></div>
				<p class="text-neutral-600 text-sm sm:text-base">Loading deposit requests...</p>
			</div>
			...
			{#each filteredRequests as request (request.id)}
				...
				>
					...
					<div class="bg-gray-50 rounded-lg p-2.5 sm:p-3">
						<div class="text-xs text-gray-500 mb-0.5 sm:mb-1">Deposit Amount</div>
