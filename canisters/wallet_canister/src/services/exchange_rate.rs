...
pub async fn convert_fiat_currency(
    ...
    let from_to_usd = get_fiat_to_usd_rate(from_currency).await?;
    let usd_amount = (amount as f64) * from_to_usd;

    // Convert USD to to_currency
    let usd_to_target = 1.0 / get_fiat_to_usd_rate(to_currency).await?;
    let target_amount = usd_amount * usd_to_target;

    Ok(target_amount as u64)
}
...
mod tests {
    ...
    #[test]
    fn test_mock_fiat_rates() {
        assert!(get_mock_fiat_to_usd_rate("KES").is_ok());
        assert!(get_mock_fiat_to_usd_rate("UGX").is_ok());
        assert!(get_mock_fiat_to_usd_rate("NGN").is_ok());
        assert!(get_mock_fiat_to_usd_rate("INVALID").is_err());
    }
}
