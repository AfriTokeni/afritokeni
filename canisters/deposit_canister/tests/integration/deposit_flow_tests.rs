...
use deposit_canister::{
    CreateDepositRequest, ConfirmDepositRequest,
    DepositTransaction, TransactionStatus
};

const WASM_PATH: &str = "../../target/wasm32-unknown-unknown/release/deposit_canister.wasm";
...
#[test]
fn test_create_deposit_request_success() {
    ...
    let response: Result<DepositTransaction, String> = 
...
#[test]
fn test_create_deposit_zero_amount_fails() {
    let (pic, canister_id) = setup();
...
#[test]
fn test_create_deposit_wrong_caller_fails() {
    let (pic, canister_id) = setup();
...
#[test]
fn test_confirm_deposit_success() {
    let (pic, canister_id) = setup();
    
    let user = Principal::from_text("aaaaa-aa").unwrap();
    let agent = Principal::from_text("2vxsx-fae").unwrap();
    
    let create_request = CreateDepositRequest {
        user_principal: user,
        agent_principal: agent,
        amount_ugx: 10000,
    };
    ...
    let confirm_response: Result<DepositTransaction, String> = 
...
#[test]
fn test_confirm_deposit_wrong_agent_fails() {
    let (pic, canister_id) = setup();
...
fn test_commission_calculation() {
    ...
    assert_eq!(tx.commission_ugx, 500);
}
...
fn test_double_confirmation_fails() {
    ...
    let confirm_request2 = ConfirmDepositRequest {
        deposit_code: tx.deposit_code.clone(),
        agent_principal: agent,
    };
    ...
    assert!(response.is_err());
    assert_eq!(response.unwrap_err(), "Deposit already processed");
}
...
fn test_deposit_code_uniqueness() {
    ...
    let request1 = CreateDepositRequest {
        user_principal: user,
        agent_principal: agent,
        amount_ugx: 10000,
    };
    
    let request2 = CreateDepositRequest {
        user_principal: user,
        agent_principal: agent,
        amount_ugx: 20000,
    };
    ...
    let tx1: Result<DepositTransaction, String> = decode_one(&result1).unwrap();
    let tx2: Result<DepositTransaction, String> = decode_one(&result2).unwrap();
    
    assert_ne!(tx1.unwrap().deposit_code, tx2.unwrap().deposit_code);
}
...
fn test_large_deposit_amount() {
    ...
    assert_eq!(tx.commission_ugx, 5000);
}
