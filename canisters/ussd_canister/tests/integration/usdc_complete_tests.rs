...
fn test_buy_usdc_with_ugx() {
    ...
    // Buy USDC: Menu 3 (USDC) -> 2 (Buy) -> amount -> PIN
...
fn test_buy_usdc_with_kes() {
    ...
    let (response, _) = env.process_ussd("session", phone, "3*2*50000*1234");
...
fn test_buy_usdc_with_tzs() {
    ...
    let (response, _) = env.process_ussd("session", phone, "3*2*200000*1234");
...
fn test_buy_usdc_with_ngn() {
    ...
    let (response, _) = env.process_ussd("session", phone, "3*2*500000*1234");
...
fn test_send_usdc_to_valid_address() {
    ...
    // Send USDC: Menu 3 -> 3 (Send) -> address -> amount -> PIN
    let (response, _) = env.process_ussd("session", phone, "3*3*0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb*50000*1234");
...
fn test_send_usdc_invalid_address() {
    ...
    let (response, _) = env.process_ussd("session", phone, "3*3*invalid_address");
...
fn test_send_usdc_zero_amount() {
    ...
    let (response, _) = env.process_ussd("session", phone, "3*3*0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb*0");
...
fn test_sell_usdc_to_ugx() {
    ...
    // Sell USDC: Menu 3 -> 4 (Sell) -> amount -> PIN
    let (response, _) = env.process_ussd("session", phone, "3*4*100000*1234");
...
fn test_sell_usdc_all_balance() {
    ...
    let (response, _) = env.process_ussd("session", phone, "3*4*150000*1234");
...
fn test_buy_usdc_wrong_pin() {
    ...
    let (response, _) = env.process_ussd("session", phone, "3*2*50000*9999");
...
fn test_sell_usdc_wrong_pin() {
    ...
    let (response, _) = env.process_ussd("session", phone, "3*4*50000*9999");
...
fn test_usdc_buy_then_sell() {
    ...
    // Buy USDC
    env.process_ussd("s1", phone, "3*2*200000*1234");
...
fn test_usdc_buy_then_send() {
    ...
    // Buy USDC
    env.process_ussd("s1", phone, "3*2*300000*1234");
    
    let (_, usdc_after_buy) = env.get_crypto_balance(phone).expect("Get balance");
    
    // Send USDC
    env.process_ussd("s2", phone, &format!("3*3*0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb*{}*1234", usdc_after_buy / 2));
...
fn test_usdc_and_btc_independent_balances() {
    ...
    env.process_ussd("s1", phone, "3*2*100000*1234");
