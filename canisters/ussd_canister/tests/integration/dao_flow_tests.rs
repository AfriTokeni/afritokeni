...
fn test_dao_menu_navigation() {
    ...
    // Register user first
    env.setup_test_user_with_balances(phone, "DAO", "Nav", "daonav@test.com", "UGX", "1234", 0, 0, 0)
        .expect("Setup");
...
fn test_dao_view_proposals() {
    ...
    // Register user first
    env.setup_test_user_with_balances(phone, "DAO", "View", "daoview@test.com", "UGX", "1234", 0, 0, 0)
        .expect("Setup");

    // Navigate to view proposals (5*1 full path)
    env.process_ussd(session_id, phone, "5");
    let (response, continue_session) = env.process_ussd(session_id, phone, "5*1");

    assert!(continue_session || !continue_session, "May or may not continue");
    assert!(response.contains("Proposal") || response.contains("No proposals") || response.contains("Active") || response.contains("coming soon"),
...
fn test_dao_vote_on_proposal() {
    ...
    // Navigate to vote (5*2 full path)
    env.process_ussd(session_id, phone, "5");
    let (response, _) = env.process_ussd(session_id, phone, "5*2");

    assert!(response.contains("Vote") || response.contains("Proposal") || response.contains("No proposals") || response.contains("coming soon"),
...
fn test_dao_menu_structure() {
    ...
    // Register user first
    env.setup_test_user_with_balances(phone, "DAO", "Structure", "daostructure@test.com", "UGX", "1234", 0, 0, 0)
        .expect("Setup");
...
fn test_dao_return_to_main_menu() {
    ...
    // Register user first
    env.setup_test_user_with_balances(phone, "DAO", "Return", "daoreturn@test.com", "UGX", "1234", 0, 0, 0)
        .expect("Setup");
