...
const USSD_MASTER_PRINCIPAL =
  process.env.VITE_USSD_MASTER_PRINCIPAL || "aaaaa-aa"; // Placeholder - will be replaced with real principal

// Secret salt for subaccount generation (MUST be kept secure)
const SUBACCOUNT_SECRET =
  process.env.VITE_USSD_SUBACCOUNT_SECRET || "afritokeni-ussd-secret-2024"; // In production, use a secure random value
 ...
 */
async function _sha256Hash(data: Uint8Array): Promise<Uint8Array> {
 ...
 */
export async function getUSSDCkBTCBalance(
  phoneNumber: string,
): Promise<bigint> {
...
export async function transferUSSDCkBTC(
  ...
  console.log(
    `   From: ${fromPhone} (subaccount: ${subaccountToHex(fromInfo.subaccount).slice(0, 8)}...)`,
  );
  console.log(
    `   To: ${toPhone} (subaccount: ${subaccountToHex(toInfo.subaccount).slice(0, 8)}...)`,
  );
