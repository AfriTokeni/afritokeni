...
export class DemoAgentService extends BaseAgentService {
  ...
  async createAgent(agent: Omit<AgentMetadata, 'id' | 'createdAt'>): Promise<AgentMetadata> {
    logger.debug('createAgent (demo mode)', { businessName: agent.businessName });
   ...
   * Complete agent KYC
   */
  async completeAgentKYC(agentKYCData: AgentKYCData): Promise<{ user: unknown; agent: AgentMetadata }> {
