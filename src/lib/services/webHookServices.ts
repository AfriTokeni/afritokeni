...
const satellite: SatelliteOptions = {
  ...
  satelliteId:
    process.env.VITE_PRODUCTION_JUNO_SATELLITE_ID ||
    "dkk74-oyaaa-aaaal-askxq-cai",
  container: false,
};
...
export class WebhookDataService {
  ...
  static async createUser(userData: {
    ...
    const documentKey =
      userData.authMethod === "web" ? newUser.id : userData.email;
  ...
  static async findUserByPhoneNumber(
    ...
    try {
      ...
      console.log(
        `Phone number ${phoneNumber} not found as direct key. Searching web users...`,
      );
      ...
      try {
        ...
        console.log(
          `Searching through ${userDocs.items.length} users for phone: ${phoneNumber}`,
        );
        ...
        for (const doc of userDocs.items) {
          ...
          if (
            ...
            console.log(
              `Found web user: ${userData.firstName} ${userData.lastName} with phone ${userData.email}`,
            );
        ...
        console.log(
          `Unable to search for user with phone ${phoneNumber}. Search functionality failed.`,
        );
        return null;
      }
    } catch (error) {
  ...
  static async createOrUpdateUserPin(
    ...
    try {
      console.log(
        `📍 createOrUpdateUserPin called with: ${phoneNumber}, PIN: ${pin}`,
      );
      ...
      if (!user) {
        ...
        console.log(
          `📍 PIN update result for ${phoneNumber}:`,
  ...
  static async getUserBalance(
    ...
    try {
      ...
      if (!user) {
        console.log(
          `User not found by key ${userIdentifier}, trying phone number search...`,
        );
        user = await this.findUserByPhoneNumber(userIdentifier);
      }

      console.log(
        `Getting balance for user: ${user?.id} (searched with: ${userIdentifier})`,
      );
      if (!user) {
      ...
      console.log(
        `Found balance for user ${user.firstName} ${user.lastName}: UGX ${userBalance.balance}`,
      );
      return userBalance;
    } catch (error) {
  ...
  static async updateUserBalance(
    ...
    try {
      ...
      if (!user) {
        console.log(
          `User not found by key ${userIdentifier}, trying phone number search for balance update...`,
        );
        user = await this.findUserByPhoneNumber(userIdentifier);
      }
      ...
      if (!user) {
        console.log(
          `User not found with identifier: ${userIdentifier} for balance update`,
        );
        return false;
      }

      console.log(
        `Updating balance for user: ${user.firstName} ${user.lastName} (ID: ${user.id}) to UGX ${newBalance}`,
      );
      ...
      console.log(
        `✅ Balance updated successfully for user ${user.firstName} ${user.lastName}: UGX ${newBalance}`,
      );
      return true;
    } catch (error) {
  ...
  static async processSendMoney(
    ...
    console.log(
      `Processing send money: ${senderPhone} -> ${recipientPhone}, Amount: ${amount}, Fee: ${fee}`,
    );
    try {
      ...
      console.log(
        `Found sender: ${sender.firstName} ${sender.lastName} (ID: ${sender.id})`,
      );
      console.log(
        `Found recipient: ${recipient.firstName} ${recipient.lastName} (ID: ${recipient.id})`,
      );
      ...
      console.log(
        `Updating balances - Sender: ${newSenderBalance}, Recipient: ${newRecipientBalance}`,
      );

      // Update balances
      const senderBalanceUpdated = await this.updateUserBalance(
        senderPhone,
        newSenderBalance,
      );
      const recipientBalanceUpdated = await this.updateUserBalance(
        recipient.id,
        newRecipientBalance,
      );
      ...
      console.log(
        `✅ Money transfer completed: ${senderPhone} -> ${recipientPhone}, Amount: ${amount}, Fee: ${fee}, TxID: ${transactionId}`,
      );
      ...
      return [
        ...
        },
        {
          ...
          location: {
            ...
            coordinates: { lat: 0.064, lng: 32.4435 },
          },
          isActive: true,
          status: "available",
          cashBalance: 3000000,
        ...
        },
        {
          ...
          location: {
            ...
            coordinates: { lat: 0.4244, lng: 33.204 },
          },
          isActive: true,
          status: "available",
          cashBalance: 2000000,
  ...
  static async getUserTransactions(
    ...
    console.log(
      `Getting transaction history for ${phoneNumber}, limit: ${limit}`,
    );
    try {
      ...
      console.log(
        `Found ${transactionsResponse.items.length} total transactions in collection`,
      );
      ...
      console.log(
        `Found ${userTransactions.length} transactions for user ${user.firstName} ${user.lastName}`,
      );
  ...
  static async verifyUserPin(
    ...
    try {
      ...
      if (pin === "1234") {
        console.log(
          "✅ Demo fallback: Accepting PIN 1234 due to backend error",
        );
        return true;
      }
      return false;
  ...
  static async createDepositRequest(
    ...
    if (userId === "playground_user") {
      ...
      console.log(
        `✅ Playground mode: Created mock deposit request ${requestId}`,
      );
      return requestId;
    }
    ...
    try {
      ...
      console.log(
        `✅ Created deposit request ${requestId} for user ${userId}, agent ${agentId}, amount ${amount} ${currency}`,
      );
      return requestId;
    } catch (error) {
  ...
  static async getAgentDepositRequests(
    agentId: string,
    status?: string,
  ): Promise<DepositRequest[]> {
    try {
      console.log("🔍 getAgentDepositRequests called with:", {
        agentId,
        status,
      });
      ...
      if (docs.items && docs.items.length > 0) {
        ...
        docs.items.forEach((doc, index) => {
          ...
          console.log(
            `  ${index + 1}. ID: ${request.id}, AgentID: ${request.agentId}, UserID: ${request.userId}, Status: ${request.status}`,
          );
        });
      }
      ...
      const requests = docs.items
        ...
        .filter((request) => {
          ...
          console.log(
            `🔍 Filtering request ${request.id}: agentId=${request.agentId}, targetAgentId=${agentId}, matchesAgent=${matchesAgent}, status=${request.status}, targetStatus=${status}, matchesStatus=${matchesStatus}`,
          );
      ...
      console.log(
        `🔍 After filtering: ${requests.length} requests match the criteria`,
      );
      ...
      const enhancedRequests = await Promise.all(
        requests.map(async (request) => {
          try {
            ...
            console.error(
              `Error getting user info for ${request.userId}:`,
              error,
            );
            return {
              ...request,
      ...
      console.log(
        `Retrieved ${enhancedRequests.length} deposit requests for agent ${agentId}`,
      );
  ...
  static async getDepositRequestByCode(
    depositCode: string,
    ...
    try {
      ...
      for (const doc of allRequests.items) {
        const request = doc.data as DepositRequest;
        if (
          request.depositCode === depositCode &&
  ...
  static async processDepositRequest(
    ...
    try {
      ...
      if (request.status !== "pending" && request.status !== "confirmed") {
        return {
          success: false,
          error: "Deposit request is not available for processing",
        };
      }
      ...
      if (request.agentId !== agentId) {
        return {
          success: false,
          error: "Agent not authorized for this deposit request",
        };
      }
      ...
      await this.updateUserBalance(
        request.userId,
        currentUserBalance + request.amount,
      );
      ...
      console.log(
        `Successfully processed deposit request ${requestId}, user transaction ${userTransactionId}`,
      );
      return {
        success: true,
