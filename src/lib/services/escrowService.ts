...
export class EscrowService {
   ...
   */
  static async verifyAndComplete(
    exchangeCode: string,
    agentId: string,
  ): Promise<EscrowTransaction> {
   ...
   */
  private static async refundTransaction(
  ...
  static async getTransaction(
    id: string,
  ): Promise<(EscrowTransaction & { _version?: bigint }) | null> {
    try {
      const doc = await getDoc({
        collection: "escrow_transactions",
        key: id,
  ...
  private static async getTransactionByCode(
    code: string,
  ): Promise<EscrowTransaction | null> {
    try {
      ...
      const transaction = results.items.find(
        (item: any) => item.data.exchangeCode === code,
      );
  ...
  private static async notifyAgent(
    agentId: string,
    transaction: EscrowTransaction,
  ): Promise<void> {
    console.log(
      `Notifying agent ${agentId} of new transaction ${transaction.id}`,
    );
  }

  private static async notifyTransactionComplete(
    transaction: EscrowTransaction,
  ): Promise<void> {
    console.log(`Transaction ${transaction.id} completed successfully`);
  }

  private static async notifyRefund(
    transaction: EscrowTransaction,
  ): Promise<void> {
    console.log(
      `Refunding transaction ${transaction.id} to user ${transaction.userId}`,
    );
  }

  private static async updateAgentStats(
    _agentId: string,
    _success: boolean,
  ): Promise<void> {
