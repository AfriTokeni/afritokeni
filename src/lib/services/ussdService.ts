...
import {
  ...
  handleBTCBalance,
  handleBTCRate,
} from "./ussd/handlers/bitcoin";
import {
  handleUSDC,
  handleUSDCBalance,
  handleUSDCRate,
} from "./ussd/handlers/usdc";
...
export class USSDService {
  ...
  static async createUSSDSession(
    sessionId: string,
    phoneNumber: string,
  ): Promise<USSDSession> {
  ...
  static async updateUSSDSession(
    sessionId: string,
    ...
    try {
      ...
      // Update the session
      if (updates.currentMenu)
        existingSession.currentMenu = updates.currentMenu;
      if (updates.data)
        existingSession.data = { ...existingSession.data, ...updates.data };
      if (updates.step !== undefined) existingSession.step = updates.step;
      if (updates.lastActivity !== undefined)
        existingSession.lastActivity = updates.lastActivity;
      if (updates.language !== undefined)
        existingSession.language = updates.language;
  ...
  static async processUSSDRequest(
    ...
    try {
      ...
      if (session.isExpired()) {
        ...
        return {
          response:
            "END " + TranslationService.translate("session_expired", lang),
      ...
      switch (session.currentMenu) {
        ...
        case "local_currency":
          response = await handleLocalCurrency(
            input,
            session,
            handleSendMoney,
            handleDeposit,
        ...
        case "language_selection": {
          ...
          if (response.includes("__SHOW_MAIN_MENU__")) {
            console.log(
              "🔄 Detected __SHOW_MAIN_MENU__ marker, showing main menu in language:",
              session.language,
            );
            response = await handleMainMenu(
              "",
      ...
      console.log(
        `💾 Saving session: menu=${session.currentMenu}, step=${session.step}, language=${session.language}`,
      );
      await this.updateUSSDSession(sessionId, {
        currentMenu: session.currentMenu,
        data: session.data,
