...
export async function getRevenueStats(): Promise<RevenueStats> {
  try {
    ...
    // Last month period
    const lastMonthTransactions = transactions.filter((tx: any) => 
    ...
    const lastMonthTotal = lastMonthDeposits + lastMonthWithdrawals + lastMonthExchange;
    ...
    return {
      ...
      depositCommissionsChange: calculateChange(currentDeposits, lastMonthDeposits),
      withdrawalFeesChange: calculateChange(currentWithdrawals, lastMonthWithdrawals),
  ...
  try {
    ...
    const deposits = dateRange.map(date => {
      return transactions
        .filter((tx: any) => {
          const txDate = new Date(tx.createdAt);
          return txDate <= date && tx.type === 'deposit' && tx.status === 'completed';
        })
        .reduce((sum: number, tx: any) => sum + (tx.fee ?? 0), 0);
    });
    ...
    const withdrawals = dateRange.map(date => {
      return transactions
        .filter((tx: any) => {
          const txDate = new Date(tx.createdAt);
          return txDate <= date && tx.type === 'withdrawal' && tx.status === 'completed';
        })
        .reduce((sum: number, tx: any) => sum + (tx.fee ?? 0), 0);
    });
