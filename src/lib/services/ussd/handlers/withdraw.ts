  ...
  switch (session.step) {
    case 1: {
      ...
      if (amount > 2000000) {
        const currency = getSessionCurrency(session);
        return continueSession(
          `${TranslationService.translate("maximum_amount", lang)}: ${currency} 2,000,000\n${TranslationService.translate("enter_amount", lang)} (${currency}):`,
        );
      }
      ...
      try {
        ...
        if (agents.length === 0) {
          return endSession(
            `${TranslationService.translate("no_agents_available", lang)}. ${TranslationService.translate("please_try_again_later", lang)}.`,
          );
        }
...
`;
    ...
    case 3: {
      ...
      if (
        ...
        return continueSession(
          `${TranslationService.translate("invalid_selection", lang)}. ${TranslationService.translate("select_an_agent", lang)} ${TranslationService.translate("or_cancel", lang)}:\n\n${TranslationService.translate("back_or_menu", lang)}`,
        );
      }
    ...
    case 4: {
      ...
      if (!sanitized_input || sanitized_input.length !== 4) {
        ...
        if (session.data.pinAttempts >= 3) {
          return endSession(
            `${TranslationService.translate("too_many_attempts", lang)}. ${TranslationService.translate("transaction_cancelled", lang)}.`,
          );
        }
      ...
      try {
        ...
        if (!pinValid) {
          ...
          if (session.data.pinAttempts >= 3) {
            return endSession(
              `${TranslationService.translate("incorrect_pin", lang)}. ${TranslationService.translate("too_many_attempts", lang)}. ${TranslationService.translate("transaction_cancelled", lang)}.`,
            );
          }
        ...
        console.log(
          `Creating withdrawal transaction for ${session.phoneNumber}`,
        );
