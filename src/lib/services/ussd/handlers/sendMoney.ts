...
${TranslationService.translate("enter_pin_to_confirm", lang)}:`);
    ...
    case 2: {
      ...
      const isValidPin = await DataService.verifyUserPin(
        `+${session.phoneNumber}`,
        currentInput,
      );
      if (!isValidPin) {
        ...
        if (session.data.pinAttempts >= 3) {
          return endSession(
            `${TranslationService.translate("too_many_attempts", lang)}.\n${TranslationService.translate("transaction_cancelled", lang)}.\n\n${TranslationService.translate("thank_you", lang)}`,
          );
        }

        return continueSession(
          `${TranslationService.translate("incorrect_pin", lang)}. ${3 - session.data.pinAttempts} ${TranslationService.translate("attempts_remaining", lang)}.\n${TranslationService.translate("enter_pin_4digit", lang)}:`,
        );
      }

      // PIN is correct, process the transaction
      console.log(
        `💳 Processing money transfer: ${session.phoneNumber} -> ${session.data.recipientPhone}, Amount: ${session.data.amount}, Fee: ${session.data.fee}`,
      );
