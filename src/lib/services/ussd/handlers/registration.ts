...
const verificationCodes = new Map<
  string,
  { code: string; userId: string; timestamp: number }
>();
...
export async function isUserRegistered(phoneNumber: string): Promise<boolean> {
  try {
    ...
    console.log(
      `Registration check result for ${phoneNumber}:`,
      user ? "User found" : "User not found",
    );
    return user !== null;
  } catch (error) {
    console.error(
      `Error checking user registration for ${phoneNumber}:`,
      error,
    );
    return false;
  }
...
export async function registerNewUser(
  ...
  try {
    console.log(
      `Registering new user: ${firstName} ${lastName} with phone ${phoneNumber}`,
    );
    ...
    console.log(
      `Successfully registered user: ${phoneNumber} with currency ${preferredCurrency}`,
    );
    return true;
  } catch (error) {
 ...
 */
export async function handleRegistrationCheck(
  input: string,
  session: USSDSession,
  sendSMS: (phone: string, msg: string) => Promise<any>,
  hasUserPin: (phone: string) => Promise<boolean>,
): Promise<string> {
  const lang = session.language || "en";
  console.log(
    `🔍 Registration check for ${session.phoneNumber}, input: "${input}"`,
  );
  ...
  if (!input) {
    ...
    console.log(
      `📋 User ${session.phoneNumber} registration status: ${isRegistered ? "Registered" : "Not registered"}`,
    );
    ...
    if (!isRegistered) {
      ...
      console.log(
        `🔑 User ${session.phoneNumber} PIN status: ${hasPIN ? "Has PIN" : "No PIN"}`,
      );
      ...
      if (!hasPIN) {
        ...
        console.log(
          `➡️ User ${session.phoneNumber} has PIN, currency: ${currency}, going directly to main menu`,
        );
        return continueSession(
...
export async function handleUserRegistration(
  ...
  console.log(
    `📝 User registration for ${session.phoneNumber}, step: ${session.step}, input: "${sanitized_input}"`,
  );
  ...
  switch (session.step) {
    case 1: {
      ...
      if (!sanitized_input || sanitized_input.trim().length < 3) {
        console.log(
          `❌ Invalid name "${sanitized_input}" provided by ${session.phoneNumber}`,
        );
        return continueSession(
          `${TranslationService.translate("invalid_name", lang)}. ${TranslationService.translate("enter_full_name", lang)}:\n\n${TranslationService.translate("back_or_menu", lang)}`,
        );
      }
      ...
      if (fullNameParts.length < 2) {
        console.log(
          `❌ Incomplete name "${sanitized_input}" provided by ${session.phoneNumber}`,
        );
        return continueSession(
          `${TranslationService.translate("enter_both_names", lang)}:\n\n${TranslationService.translate("back_or_menu", lang)}`,
        );
      }
      ...
      console.log(
        `✅ Full name "${sanitized_input}" split into firstName: "${firstName}", lastName: "${lastName}" for ${session.phoneNumber}`,
      );
      ...
      console.log(
        `🔢 Generated verification code ${verificationCode} for ${session.phoneNumber}`,
      );
      ...
      try {
        ...
        session.step = 1;
        console.log(
          `📱 SMS sent successfully to ${session.phoneNumber}, moving to verification menu`,
        );
        return continueSession(
          `${TranslationService.translate("thank_you", lang)} ${firstName} ${lastName}!\n${TranslationService.translate("verification_code_sent", lang)}\n${TranslationService.translate("enter_verification_code", lang)}:\n\n${TranslationService.translate("back_or_menu", lang)}`,
        );
      } catch (error) {
        console.error(
          `❌ Failed to send verification SMS to ${session.phoneNumber}:`,
          error,
        );
        return endSession(
          `${TranslationService.translate("verification_failed", lang)}`,
        );
      }
    }
    ...
    default:
      console.log(
        `❌ Invalid registration step ${session.step} for ${session.phoneNumber}`,
      );
  ...
  switch (session.step) {
    case 1: {
      ...
      if (!sanitized_input || sanitized_input.length !== 6) {
        session.data.verificationAttempts =
          (session.data.verificationAttempts || 0) + 1;
        console.log(
          `❌ Invalid code format for ${session.phoneNumber}, attempts: ${session.data.verificationAttempts}`,
        );
        if (session.data.verificationAttempts >= 3) {
          console.log(
            `🚫 Max verification attempts reached for ${session.phoneNumber}`,
          );
          return endSession(
        ...
        return continueSession(
          `${TranslationService.translate("invalid_code_format", lang)}. ${TranslationService.translate("enter_verification_code", lang)}:\n\n${TranslationService.translate("back_or_menu", lang)}`,
        );
      }
      ...
      console.log(
        `🔐 Code verification result for ${session.phoneNumber}: ${isValidCode ? "Valid" : "Invalid"}`,
      );
      ...
      if (!isValidCode) {
        session.data.verificationAttempts =
          (session.data.verificationAttempts || 0) + 1;
        console.log(
          `❌ Invalid verification code for ${session.phoneNumber}, attempts: ${session.data.verificationAttempts}`,
        );
        if (session.data.verificationAttempts >= 3) {
          console.log(
            `🚫 Max verification attempts reached for ${session.phoneNumber}`,
        ...
        return continueSession(
          `${TranslationService.translate("invalid_verification_code", lang)}. ${TranslationService.translate("try_again", lang)}:\n\n${TranslationService.translate("back_or_menu", lang)}`,
        );
      }

      // Code is valid - register the user
      console.log(
        `✅ Valid verification code for ${session.phoneNumber}, proceeding with user registration`,
      );
      ...
      if (!registrationSuccess) {
        ...
        return endSession(
          TranslationService.translate("registration_failed", lang),
        );
      }

      // Registration successful - now set up PIN
      console.log(
        `👤 User ${session.phoneNumber} registered successfully, moving to PIN setup`,
      );
      session.currentMenu = "pin_setup";
      session.step = 1;
      return continueSession(
        `${TranslationService.translate("verification_successful", lang)}!\n${TranslationService.translate("account_created", lang)}.\n${TranslationService.translate("setup_pin_message", lang)}:\n${TranslationService.translate("enter_new_pin", lang)}:\n\n${TranslationService.translate("back_or_menu", lang)}`,
      );
    }

    default:
      console.log(
        `❌ Invalid verification step ${session.step} for ${session.phoneNumber}`,
      );
      return endSession(
        TranslationService.translate("verification_failed", lang),
      );
