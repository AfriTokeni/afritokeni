...
6. Language Selection`);
  ...
  console.log(
    `Main menu sanitized input: "${sanitized_input}" (from ${inputParts.length} parts, chained: ${isChainedInput})`,
  );
  ...
  switch (sanitized_input) {
    ...
    case "4":
      ...
      return continueSession(
        `${TranslationService.translate("dao_governance", lang)} ${TranslationService.translate("coming_soon", lang)}!\n\n${TranslationService.translate("back_or_menu", lang)}`,
      );
