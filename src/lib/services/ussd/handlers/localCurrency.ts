...
import { requestPinVerification, requiresPinVerification, verifyUserPin } from "./pinManagement.js";
...
export async function handleCheckBalance(
  ...
  if (session.data.pinVerified) {
    console.log(
      `PIN already verified for ${session.phoneNumber}, showing balance directly`,
    );
    try {
      const currency = getSessionCurrency(session);
      ...
      return endSession(
        `${TranslationService.translate("error_try_again", session.language || "en")}\n\n${TranslationService.translate("thank_you", session.language || "en")}`,
      );
    }
  }
...
export async function handleTransactionHistory(
  ...
  if (session.data.pinVerified && session.step === 1) {
    console.log(
      `PIN already verified for ${session.phoneNumber}, showing transaction history directly`,
    );
    try {
      const currency = getSessionCurrency(session);
      ...
      return endSession(
        `${TranslationService.translate("error_retrieving_history", lang)}\n${TranslationService.translate("please_try_again_later", lang)}\n\n${TranslationService.translate("thank_you", lang)}`,
      );
    }
  }
  ...
  switch (session.step) {
    ...
    default:
      return endSession(
        `${TranslationService.translate("error_retrieving_history", lang)} ${TranslationService.translate("please_try_again", lang)}`,
      );
  }
}
