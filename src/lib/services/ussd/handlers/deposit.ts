...
export async function handleDeposit(
  ...
  switch (session.step) {
    case 1: {
      ...
      if (amount < 1000) {
        return continueSession(
          `${TranslationService.translate("minimum", lang)} ${TranslationService.translate("deposit", lang)}: ${currency} 1,000\n${TranslationService.translate("enter_amount", lang)} (${currency}):`,
        );
      }

      if (amount > 5000000) {
        return continueSession(
          `Maximum ${TranslationService.translate("deposit", lang)}: ${currency} 5,000,000\n${TranslationService.translate("enter_amount", lang)} (${currency}):`,
        );
      }
...
`;
    ...
    case 2: {
      ...
      if (agentChoice === 0) {
        return endSession(
          `${TranslationService.translate("deposit", lang)} ${TranslationService.translate("transaction_cancelled", lang)}.\n\n${TranslationService.translate("thank_you", lang)}`,
        );
      }
      ...
      if (
        ...
        return continueSession(
          `${TranslationService.translate("invalid_selection", lang)}. ${TranslationService.translate("select_agent", lang)}:\n\n${TranslationService.translate("back_or_menu", lang)}`,
        );
      }
    ...
    case 3: {
      ...
      if (!currentInput || currentInput.length !== 4) {
        ...
        if (session.data.pinAttempts >= 3) {
          return endSession(
            `${TranslationService.translate("too_many_attempts", lang)}. ${TranslationService.translate("deposit", lang)} ${TranslationService.translate("transaction_cancelled", lang)}.`,
          );
        }
      ...
      try {
        const pinValid = await DataService.verifyUserPin(
          session.phoneNumber,
          currentInput,
        );
        ...
        if (!pinValid) {
          ...
          if (session.data.pinAttempts >= 3) {
            return endSession(
              `${TranslationService.translate("incorrect_pin", lang)}. ${TranslationService.translate("too_many_attempts", lang)}. ${TranslationService.translate("deposit", lang)} ${TranslationService.translate("transaction_cancelled", lang)}.`,
            );
          }

          return continueSession(
            `${TranslationService.translate("incorrect_pin", lang)}. ${TranslationService.translate("try_again", lang)}:\n${TranslationService.translate("attempts_remaining", lang)}: ${3 - session.data.pinAttempts}`,
          );
        }
        ...
        if (!selectedAgent) {
          return endSession(
            `${TranslationService.translate("select_agent", lang)}. ${TranslationService.translate("please_try_again", lang)}`,
          );
        }
