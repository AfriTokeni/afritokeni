 ...
 */
async function handleViewProposals(
  input: string,
  session: USSDSession,
): Promise<string> {
  const lang = session.language || "en";
...
0. ${TranslationService.translate("back_to_main_menu", lang)}`);
    ...
    const existingVote = userVotes.find(
      (v: any) => v.proposalId === proposal.id,
    );
...
${TranslationService.translate("thank_you_governance", lang)}`);
    ...
    const daysLeft = Math.ceil(
      (votingEndsAt.getTime() - Date.now()) / (24 * 60 * 60 * 1000),
    );
...
${TranslationService.translate("enter_pin_to_confirm", lang)}:`);
  ...
  if (session.step === 4) {
    ...
    try {
      pinCorrect = await DataService.verifyUserPin(
        phoneNumber,
        sanitized_input,
      );
    } catch (error) {
    ...
    session.data.lockedTokens =
      (session.data.lockedTokens || 0) + session.data.voteAmount;
 ...
 */
async function handleVotingPower(
  input: string,
  session: USSDSession,
): Promise<string> {
  const lang = session.language || "en";
 ...
 */
async function handleActiveVotes(
  input: string,
  session: USSDSession,
): Promise<string> {
  const lang = session.language || "en";
