...
async function ensurePrincipalId(user: any): Promise<string> {
  ...
  console.log(
    `⚠️ CRITICAL: User ${user.id} missing Principal ID - generated: ${principalId}`,
  );
  ...
  try {
    ...
    console.error(
      `❌ Failed to update user ${user.id} with Principal ID:`,
      error,
    );
  }
...
async function safeGetBalance(principalId: string, currency: string) {
  ...
  return await CkBTCService.getBalanceWithLocalCurrency(
    principalId,
    currency,
    true,
  );
...
let sendSMSNotification: (phone: string, msg: string) => Promise<any> = async (
  phone: string,
  msg: string,
) => {
  console.log(
    "🎭 Playground: Mock SMS to",
    phone,
    ":",
    msg.substring(0, 50) + "...",
  );
...
async function handleBitcoin(
  ...
  console.log(
    `🔵 handleBitcoin called - input: "${input}", currentInput: "${currentInput}", session.currentMenu: ${session.currentMenu}, session.step: ${session.step}`,
  );
...
0. ${TranslationService.translate("back_to_main_menu", lang)}`);
  ...
  switch (currentInput) {
    case "1":
      console.log(
        `🟢 Changing menu from ${session.currentMenu} to btc_balance`,
      );
      session.currentMenu = "btc_balance";
      session.step = 1;
      console.log(
        `🟢 Menu changed to: ${session.currentMenu}, step: ${session.step}`,
      );
      console.log(
        `🔍 Input analysis: inputParts=${JSON.stringify(inputParts)}, length=${inputParts.length}`,
      );
      console.log(
        `🔍 inputParts[length-2]="${inputParts[inputParts.length - 2]}", pinVerified=${session.data.pinVerified}`,
      );
      ...
      if (inputParts.length > 2 && inputParts[inputParts.length - 2] === "1") {
        console.log(
          `✅ PIN in same request detected, calling handleBTCBalance`,
        );
        return await handleBTCBalance(input, session);
      }
...
async function handleBTCBalance(
  input: string,
  session: USSDSession,
): Promise<string> {
  const lang = session.language || "en";
    ...
    case 1: {
      ...
      try {
        ...
        if (!user) {
          ...
          return endSession(
            TranslationService.getDemoMessage("btc_balance", lang, {
...
async function handleBTCRate(
  ...
  if (session.step === 2) {
    ...
    try {
      const exchangeRate = await safeGetExchangeRate(
...
async function handleBTCBuy(
  ...
  switch (session.step) {
    ...
    case 2: {
      ...
      if (ugxAmount < 10000) {
        return continueSession(
          `${TranslationService.translate("minimum_purchase", lang)}: ${currency} 10,000\n${TranslationService.translate("enter_amount_to_spend", lang)} (${currency}):`,
        );
      }

      // Check user balance first
      const userBalance = await DataService.getUserBalance(
        `+${session.phoneNumber}`,
      );
      ...
      try {
        ...
        const availableAgents = agents.filter(
          (agent: Agent) => agent.isActive,
          // agent.cashBalance >= ugxAmount
        );

        if (availableAgents.length === 0) {
          return endSession(
            `${TranslationService.translate("no_agents_available", lang)}\n\n${TranslationService.translate("thank_you", lang)}`,
          );
        }
    ...
    case 3: {
      ...
      try {
        ...
        if (!user) {
          ...
          return endSession(
            TranslationService.getDemoMessage("btc_buy", lang, {
              currency: getSessionCurrency(session),
        ...
        const purchaseCode = Math.random()
          .toString(36)
          .substring(2, 8)
          .toUpperCase();
...
${TranslationService.translate("enter_pin_4digit", lang)}:`);
    ...
    case 2: {
      ...
      if (btcAmount < 0.00001) {
        return continueSession(
          `${TranslationService.translate("minimum_amount", lang)}: ₿0.00001 BTC\n${TranslationService.translate("enter_btc_amount", lang)}:`,
        );
      }
    ...
    case 3: {
      ...
      try {
        ...
        if (!user) {
          ...
          return endSession(
            TranslationService.getDemoMessage("btc_sell", lang, {
              currency: getSessionCurrency(session),
        ...
        if (exchangeResult.success && exchangeResult.transactionId) {
          ...
          return endSession(
            `✅ ${TranslationService.translate("sale_initiated", lang)}\n\n${TranslationService.translate("sale_code", lang)}: ${saleCode}\n${TranslationService.translate("transaction_id", lang)}: ${exchangeResult.transactionId}\n${TranslationService.translate("selling", lang)}: ₿${btcAmount.toFixed(8)} BTC\n${TranslationService.translate("you_will_receive", lang)}: ${getSessionCurrency(session)} ${ugxAmount.toLocaleString()}\n\n${TranslationService.translate("agent", lang)}: ${selectedAgent.businessName}\n${TranslationService.translate("location", lang)}: ${selectedAgent.location?.city || TranslationService.translate("location", lang)}\n\n${TranslationService.translate("give_code_to_agent", lang)}.\n\n${TranslationService.translate("sms_sent", lang)}.\n\n${TranslationService.translate("thank_you", lang)}`,
          );
        } else {
          return endSession(
            `❌ ${TranslationService.translate("sale_failed", lang)}: ${exchangeResult.error || TranslationService.translate("error_try_again", lang)}\n\n${TranslationService.translate("please_try_again_later", lang)}.\n\n${TranslationService.translate("thank_you", lang)}`,
          );
        }
      } catch (error) {
        console.error("Error processing Bitcoin sale:", error);
        return endSession(
          TranslationService.translate("error_processing_sale", lang),
        );
      }
    }
  ...
  switch (session.step) {
    case 1: {
      ...
      try {
        ...
        if (!user) {
          return endSession(
            `${TranslationService.translate("user_not_found", lang)}`,
          );
        }
    ...
    case 2: {
      ...
      if (!recipientPhone.startsWith("+")) {
        if (recipientPhone.startsWith("256")) {
          ...
          return continueSession(
            `${TranslationService.translate("invalid_phone_format", lang)}.\n${TranslationService.translate("enter_recipient_phone", lang)}:\n${TranslationService.translate("phone_format_example", lang)}\n\n${TranslationService.translate("press_zero_back", lang)}\n\n${TranslationService.translate("back_or_menu", lang)}`,
          );
        }
      }
      ...
      try {
        const recipientUser =
          await DataService.findUserByPhoneNumber(recipientPhone);
        if (!recipientUser) {
          return continueSession(
            `${TranslationService.translate("recipient_not_found", lang)}: ${recipientPhone}.\n${TranslationService.translate("they_need_register", lang)}.\n\n${TranslationService.translate("enter_different_phone", lang)}:\n${TranslationService.translate("phone_format_example", lang)}\n\n${TranslationService.translate("back_or_menu", lang)}`,
          );
        }
    ...
    case 4: {
      ...
      try {
        ...
        if (!user) {
          ...
          return endSession(
            TranslationService.getDemoMessage("btc_send", lang, {
              btc: btcAmount.toString(),
          ...
          try {
            ...
            if (session.data.recipientPhone) {
              await sendSMSNotification(
                session.data.recipientPhone.replace("+", ""),
                recipientSMS,
              );
            }
          } catch (smsError) {
...
${TranslationService.translate("thank_you", lang)}`);
        ...
        return endSession(
          TranslationService.translate("error_processing_send", lang),
        );
      }
    }
