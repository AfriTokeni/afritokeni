 ...
 * @param agentPrincipal - Agent's ICP principal
 */
export async function getPendingDeposits(agentPrincipal: string): Promise<DepositTransaction[]> {
 ...
 * QUERY (read-only)
 */
export async function getAgentDeposits(agentPrincipal: string): Promise<DepositTransaction[]> {
	const actor = await createDepositActor();
 ...
 */
export async function getAgentBalance(agentPrincipal: string): Promise<AgentBalance | null> {
	const actor = await createDepositActor();
	const result = await actor.get_agent_balance(Principal.fromText(agentPrincipal));
 ...
 * QUERY (read-only)
 */
export async function getUserDeposits(userPrincipal: string): Promise<DepositTransaction[]> {
	const actor = await createDepositActor();
...
export function formatAgentBalance(balance: AgentBalance) {
	return {
		...
		lastSettlementDate: balance.last_settlement_date.length > 0 
