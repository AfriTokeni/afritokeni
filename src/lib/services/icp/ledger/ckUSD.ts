...
export class CkUSDService {
  ...
  static async getBalance(principalId: string): Promise<CkUSDBalance> {
    try {
      ...
      console.log(
        "🔒 NON-CUSTODIAL: Reading balance from user's principal (public ledger)",
      );
  ...
  static async transfer(
    ...
    try {
      ...
      console.log(
        "🔒 SECURITY: This requires USER signature via Internet Identity",
      );
      console.log(
        "🔒 Transfer is DIRECT: user→recipient (AfriTokeni NOT involved)",
      );
  ...
  static async getDepositAddress(
    principalId: string,
  ): Promise<EthereumDepositAddress> {
    try {
