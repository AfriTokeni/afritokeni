...
export class FraudDetectionService {
  ...
  static async checkTransaction(
    ...
    const recentTransactions = pattern.timestamps.filter(
      (t) => now.getTime() - t.getTime() < this.THRESHOLDS.rapidFireWindow,
    );
    ...
    const transactionsLastHour = pattern.timestamps.filter(
      (t) => t > hourAgo,
    ).length;
    ...
    const transactionsLastDay = pattern.timestamps.filter(
      (t) => t > dayAgo,
    ).length;
    ...
    if (recipient) {
      ...
      if (
        recipientsLastHour.length >= this.THRESHOLDS.maxUniqueRecipientsPerHour
      ) {
        return {
          isSuspicious: true,
  ...
  static async getRiskScore(phoneNumber: string): Promise<number> {
    ...
    const largeTransactions = pattern.amounts.filter(
      (amt) => amt > 1000000,
    ).length;
