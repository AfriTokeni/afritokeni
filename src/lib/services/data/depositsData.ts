...
export async function fetchAgentDepositRequests(
  ...
  try {
    const deposits =
      await DepositCanisterService.getPendingDeposits(agentPrincipal);

    // Transform canister data to UI format
    return deposits.map((deposit) => {
      const formatted =
        DepositCanisterService.formatDepositTransaction(deposit);
...
export async function fetchAgentCommissionBalance(
  ...
  try {
    const balance =
      await DepositCanisterService.getAgentBalance(agentPrincipal);
