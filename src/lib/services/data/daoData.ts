 ...
 */
export async function fetchDAOProposals(isDemoMode: boolean, limit: number = 10): Promise<any[]> {
	if (isDemoMode) {
		try {
	...
	try {
		console.log('🔄 Fetching proposals from SNS Governance canister:', CANISTER_IDS.SNS_GOVERNANCE);
		...
		const proposals = (proposalsResponse.proposals || proposalsResponse || []).map((p: any) => {
			...
			return {
				...
				createdAt: new Date(Number(p.proposalCreationTimestampSeconds || 0) * 1000),
				votingEndsAt: new Date(Date.now() + DAO_CONFIG.VOTING_PERIOD_DAYS * 24 * 60 * 60 * 1000),
				status: 'active',
				votes: {
		...
		console.log('💡 Tip: Toggle Demo Mode to see proposal data while troubleshooting');
		return [];
	}
}
...
export async function voteOnProposal(
	...
	if (isDemoMode) {
		...
		console.log(`🗳️ Demo vote recorded: ${vote.toUpperCase()} on ${proposalId} with ${votingPower} AFRI`);
