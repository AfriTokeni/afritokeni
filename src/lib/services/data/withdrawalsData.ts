...
export async function fetchAgentWithdrawalRequests(
  ...
  try {
    const withdrawals =
      await WithdrawalCanisterService.getPendingWithdrawals(agentPrincipal);

    // Transform canister data to UI format
    return withdrawals.map((withdrawal) => {
      const formatted =
        WithdrawalCanisterService.formatWithdrawalTransaction(withdrawal);
      ...
      return {
        ...
        status: formatted.status.toLowerCase() as
          | "pending"
          | "confirmed"
          | "completed"
          | "rejected",
...
export async function fetchAgentFeeEarnings(
  ...
  try {
    const earnings =
      await WithdrawalCanisterService.getAgentEarnings(agentPrincipal);
