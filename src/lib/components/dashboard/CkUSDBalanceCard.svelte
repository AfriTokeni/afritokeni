...
<script lang="ts">
  ...
  async function fetchBalance() {
    try {
      ...
      const { getCkUSDBalance } = await import(
  ...
  >
    <div class="mb-3 flex items-center justify-between sm:mb-4">
      ...
      <div class="rounded-full bg-red-50 p-1.5 sm:p-2">
        <DollarSign class="h-5 w-5 shrink-0 text-red-500 sm:h-6 sm:w-6" />
      </div>
    </div>
...
{:else}
  <div
    class="rounded-xl border border-green-200 bg-linear-to-br from-green-50 to-emerald-50 p-4 shadow-sm sm:p-5 md:p-6"
  >
    <!-- Header -->
    <div class="mb-3 flex items-center justify-between sm:mb-4">
      <div>
      ...
      <div class="flex items-center gap-1 sm:gap-2">
        <button
          ...
          class="rounded-lg p-1.5 transition-colors hover:bg-green-100 disabled:opacity-50 sm:p-2"
          title="Refresh balance"
        >
          <RefreshCw
            class="h-4 w-4 shrink-0 text-green-600 sm:h-5 sm:w-5 {isRefreshing
              ? 'animate-spin'
              : ''}"
          />
        </button>
        <div class="rounded-full bg-green-100 p-1.5 sm:p-2">
          <DollarSign class="h-5 w-5 shrink-0 text-green-600 sm:h-6 sm:w-6" />
        </div>
      </div>
    </div>
    ...
    <!-- Info Badge -->
    <div
      class="mb-3 rounded-lg border border-green-200 bg-white/60 p-2.5 sm:mb-4 sm:p-3"
    >
      <p class="text-xs text-neutral-700 sm:text-sm">
        <span class="font-semibold">Stable Value:</span> ckUSD is pegged 1:1 with
        USD.
      </p>
    </div>
    ...
    {#if showActions}
      <div class="grid grid-cols-3 gap-1.5 sm:gap-2">
        <button
          onclick={onDeposit}
          class="flex flex-col items-center gap-0.5 rounded-lg border border-green-200 bg-white p-2 transition-colors hover:bg-green-50 sm:gap-1 sm:p-2.5 md:p-3"
        >
          <Download class="h-4 w-4 shrink-0 text-green-600 sm:h-5 sm:w-5" />
        ...
        <button
          ...
          class="flex flex-col items-center gap-0.5 rounded-lg border border-green-200 bg-white p-2 transition-colors hover:bg-green-50 sm:gap-1 sm:p-2.5 md:p-3"
        >
          <Send class="h-4 w-4 shrink-0 text-green-600 sm:h-5 sm:w-5" />
          <span class="text-[10px] font-medium text-neutral-900 sm:text-xs"
            >Send</span
          >
        </button>
        ...
        <button
          ...
          class="flex flex-col items-center gap-0.5 rounded-lg border border-green-200 bg-white p-2 transition-colors hover:bg-green-50 sm:gap-1 sm:p-2.5 md:p-3"
        >
          <RefreshCw class="h-4 w-4 shrink-0 text-green-600 sm:h-5 sm:w-5" />
          <span class="text-[10px] font-medium text-neutral-900 sm:text-xs"
            >Exchange</span
          >
        </button>
