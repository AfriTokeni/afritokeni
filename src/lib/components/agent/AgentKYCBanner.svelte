...
<div
  class="{isCritical
    ...
    : 'border-orange-500 bg-orange-50'} mb-6 rounded-lg border-l-4 p-4 shadow-sm"
>
  <div class="flex items-start">
    ...
    <div class="ml-3 flex-1">
      ...
      >
        ...
        {#if kycStatus === "verified" && missingFields.length > 0}Complete Your
          Agent Profile{/if}
      </h3>
      <div
        class="mt-2 text-sm {isCritical ? 'text-red-700' : 'text-orange-700'}"
      >
        {#if kycStatus === "not_started"}
          ...
          <p class="mb-2">
            Complete KYC verification to start accepting deposits and processing
            withdrawals:
          </p>
        ...
        {:else if kycStatus === "pending"}
          <p class="mb-2 font-semibold">
            ⚠️ Your KYC verification is being reviewed. You cannot process
            transactions yet.
          </p>
          <p>
        ...
        {:else if kycStatus === "rejected"}
          <p class="mb-2 font-semibold">
            ⚠️ Your KYC verification was rejected. You cannot process
            transactions.
          </p>
          <p class="mb-2">
            Please review the feedback and resubmit your documents with the
            correct information.
          </p>
        {:else if kycStatus === "verified" && missingFields.length > 0}
          <p class="mb-2">
            You're missing some important information. Complete your profile for
            better service:
          </p>
          <ul class="list-inside list-disc space-y-1">
      ...
      <div class="mt-4 flex gap-3">
        {#if kycStatus !== "pending"}
          <button
            ...
            class="inline-flex items-center gap-2 px-4 py-2 {isCritical
              ? 'bg-red-600 hover:bg-red-700'
              : 'bg-orange-600 hover:bg-orange-700'} rounded-lg text-sm font-medium text-white transition-colors"
          >
        ...
        <button
          ...
          class="inline-flex items-center border px-4 py-2 {isCritical
            ? 'border-red-300 text-red-700 hover:bg-red-100'
            : 'border-orange-300 text-orange-700 hover:bg-orange-100'} rounded-lg text-sm font-medium transition-colors"
        >
          Go to Settings
        </button>
      </div>
