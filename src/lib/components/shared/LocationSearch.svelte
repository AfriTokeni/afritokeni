...
<div class="relative" bind:this={containerRef}>
  <div class="relative">
    ...
    <input
      ...
      {placeholder}
      {required}
      class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 pr-10 pl-10 placeholder-neutral-500 focus:border-neutral-500 focus:ring-2 focus:ring-neutral-500 focus:outline-none"
    />
    {#if query}
  ...
  {#if isOpen}
    <div
      class="ring-opacity-5 absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black"
    >
      {#if isLoading}
      ...
      {:else if suggestions.length > 0}
        {#each suggestions as suggestion (suggestion.place_id)}
          <button
            ...
            class="flex w-full items-center px-4 py-2 text-left text-sm text-neutral-700 hover:bg-neutral-50 focus:bg-neutral-50 focus:outline-none"
          >
            <MapPin class="mr-3 h-4 w-4 shrink-0 text-neutral-400" />
            <div class="truncate">
