...
export const mockProposals: Proposal[] = [
  {
    ...
    description:
      "Proposal to reduce platform transaction fees from 0.5% to 0.3% to increase competitiveness",
    category: "Economic",
    status: "active",
    votesYes: 45000,
    votesNo: 12000,
  ...
  },
  {
    ...
    description:
      "Require all agents to complete KYC verification before processing transactions",
    category: "Security",
    status: "passed",
    votesYes: 52000,
    votesNo: 5000,
...
export function getProposalStatus(proposal: Proposal): string {
  if (proposal.status === "active") {
    ...
    const daysLeft = Math.ceil(
      (proposal.endsAt.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
    );
