<script lang="ts">
  ...
  // Mock system data
  let canisters = $state([
    { name: 'Deposit Canister', id: '453vh-eqaaa-aaaac-qctia-cai', cycles: 5.2, status: 'healthy', uptime: '99.9%' },
    { name: 'Withdrawal Canister', id: '422tt-jiaaa-aaaac-qctiq-cai', cycles: 4.8, status: 'healthy', uptime: '99.8%' },
    { name: 'Exchange Canister', id: '4tzyp-7aaaa-aaaac-qctja-cai', cycles: 3.9, status: 'warning', uptime: '99.5%' },
  ]);
  ...
  function getStatusColor(status: string) {
    if (status === 'healthy' || status === 'operational') return 'bg-green-100 text-green-800';
    if (status === 'warning') return 'bg-yellow-100 text-yellow-800';
    if (status === 'error' || status === 'degraded') return 'bg-red-100 text-red-800';
...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="mt-2 text-2xl font-bold text-green-600 sm:text-3xl">Healthy</p>
        </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="mt-2 font-mono text-2xl font-bold text-gray-900 sm:text-3xl">{systemHealth.uptime}</p>
        </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="mt-2 font-mono text-2xl font-bold text-gray-900 sm:text-3xl">{systemHealth.totalCycles}T</p>
        </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="mt-2 font-mono text-2xl font-bold text-gray-900 sm:text-3xl">{canisters.length}</p>
        </div>
  ...
  <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
    <div class="mb-4 sm:mb-6">
      <h3 class="text-base font-semibold text-gray-900 sm:text-lg">Canister Status</h3>
      <p class="text-xs text-gray-500 sm:text-sm">Monitor canister health and cycles</p>
    </div>
          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center space-x-2">
                ...
                <span class="rounded-full px-2 py-1 text-xs font-medium {getStatusColor(canister.status)}">
                  {canister.status}
                </span>
              </div>
            ...
            <div class="text-right">
              ...
              <p class="font-mono text-lg font-bold text-gray-900">{canister.cycles}T</p>
            </div>
          </div>
          <div class="mt-3 border-t border-gray-100 pt-3">
            <div class="flex items-center justify-between">
              <span class="text-sm text-gray-500">Uptime</span>
              <span class="font-mono text-sm font-semibold text-gray-900">{canister.uptime}</span>
            </div>
          </div>
        </div>
      {/each}
  ...
  <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
    <div class="mb-4 sm:mb-6">
      <h3 class="text-base font-semibold text-gray-900 sm:text-lg">API Status</h3>
        ...
        <div class="rounded-lg border border-gray-100 p-4">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="mt-1 text-xs text-gray-500">Response: {api.responseTime}</p>
            </div>
            <span class="rounded-full px-2 py-1 text-xs font-medium {getStatusColor(api.status)}">
              {api.status}
            </span>
          </div>
        </div>
  ...
  <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
    <div class="mb-4 sm:mb-6">
      <h3 class="text-base font-semibold text-gray-900 sm:text-lg">Recent Logs</h3>
      ...
      {#each errorLogs as log}
        <div class="rounded-lg border border-gray-100 p-3">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <div class="flex items-center space-x-2">
                <span class="rounded-full px-2 py-1 text-xs font-medium {getLogLevelColor(log.level)}">
                  {log.level}
                </span>
  ...
  <div class="flex items-start space-x-2 rounded-lg border border-blue-200 bg-blue-50 p-3 sm:p-4">
    ...
    <div class="text-xs text-blue-900 sm:text-sm">
      ...
      <p class="mt-1 text-blue-800">
        Last deployment: {systemHealth.lastDeployment}. Monitor canister cycles and top up when below 2T to ensure continuous operation.
      </p>
    </div>
  </div>
</div>
