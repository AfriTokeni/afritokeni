<script lang="ts">
  import { TrendingUp, DollarSign, CreditCard, Banknote, ArrowUpRight, Info } from 'lucide-svelte';
  
  // Mock revenue data
  let revenueStats = $state({
    totalRevenue: 45678.50,
    depositCommissions: 28450.30,
    withdrawalFees: 12340.20,
    exchangeSpread: 4888.00,
    growth: 12.5,
  });
  
  let revenueBreakdown = $state([
    { source: 'Deposit Commissions', amount: 28450.30, percentage: 62, transactions: 1234, color: 'blue' },
    { source: 'Withdrawal Fees', amount: 12340.20, percentage: 27, transactions: 856, color: 'purple' },
    { source: 'Exchange Spread', amount: 4888.00, percentage: 11, transactions: 432, color: 'green' },
  ]);
  
  let topAgents = $state([
    { name: 'Agent Lagos Central', revenue: 12500, transactions: 234, commission: 625 },
    { name: 'Agent Nairobi East', revenue: 10200, transactions: 198, commission: 510 },
    { name: 'Agent Accra West', revenue: 9800, transactions: 176, commission: 490 },
    { name: 'Agent Kampala North', revenue: 7600, transactions: 145, commission: 380 },
  ]);
...
<div class="space-y-4 sm:space-y-6">
  ...
  <div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-4">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
      <div class="mb-3 flex items-start justify-between">
        <div class="flex-1">
          <div class="mb-2 flex items-center space-x-2">
            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-blue-50 sm:h-12 sm:w-12 sm:rounded-xl">
      ...
      <div class="flex items-center space-x-1 border-t border-gray-100 pt-3">
        ...
        <span class="text-sm font-medium text-green-600">+{revenueStats.growth}%</span>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
      <div class="mb-3 flex items-start justify-between">
        <div class="flex-1">
          <div class="mb-2 flex items-center space-x-2">
            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-purple-50 sm:h-12 sm:w-12 sm:rounded-xl">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
      <div class="mb-3 flex items-start justify-between">
        <div class="flex-1">
          <div class="mb-2 flex items-center space-x-2">
            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-green-50 sm:h-12 sm:w-12 sm:rounded-xl">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
      <div class="mb-3 flex items-start justify-between">
        <div class="flex-1">
          <div class="mb-2 flex items-center space-x-2">
            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-yellow-50 sm:h-12 sm:w-12 sm:rounded-xl">
  ...
  <!-- Revenue Breakdown -->
  <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6 lg:p-8">
    <div class="mb-4 flex items-center justify-between sm:mb-6">
      <div>
        <h3 class="text-base font-semibold text-gray-900 sm:text-lg">Revenue Sources</h3>
        <p class="text-xs text-gray-500 sm:text-sm">Breakdown by transaction type</p>
      </div>
    </div>
        ...
        <div>
          <div class="mb-2 flex items-center justify-between">
            ...
            <div class="text-right">
              <p class="font-mono text-sm font-bold text-gray-900">${item.amount.toLocaleString()}</p>
              <p class="text-xs text-gray-500">{item.transactions} transactions</p>
            </div>
          </div>
  ...
  <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
    <div class="mb-4 flex items-center justify-between sm:mb-6">
      <div>
        <h3 class="text-base font-semibold text-gray-900 sm:text-lg">Top Revenue Agents</h3>
        <p class="text-xs text-gray-500 sm:text-sm">Highest earning agents</p>
      </div>
      <a href="/admin/agents" class="text-sm font-medium text-blue-600 hover:text-blue-700">
        View all →
      </a>
    </div>
      ...
      {#each topAgents as agent, i}
        <div class="flex items-center justify-between rounded-lg border border-gray-100 p-3 transition-all hover:border-gray-200 sm:p-4">
          <div class="flex items-center space-x-3 sm:space-x-4">
            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-lg font-bold text-blue-600 sm:h-12 sm:w-12">
              {i + 1}
            </div>
            <div class="min-w-0">
              <p class="truncate text-sm font-semibold text-gray-900 sm:text-base">{agent.name}</p>
              <p class="text-xs text-gray-500">{agent.transactions} transactions</p>
            </div>
          </div>
          <div class="text-right">
  ...
  <div class="rounded-xl border border-gray-200 bg-white p-4 transition-all hover:border-gray-300 sm:rounded-2xl sm:p-6">
    ...
    <div class="space-y-3 sm:space-y-4">
      {#each monthlyRevenue as month}
        <div class="flex items-center justify-between rounded-lg border border-gray-100 p-3 sm:p-4">
          <div>
          ...
          <div class="flex items-center space-x-4">
            ...
            <div class="flex items-center space-x-1">
              <TrendingUp class="h-4 w-4 text-green-600" />
              <span class="text-sm font-medium text-green-600">+{month.growth}%</span>
            </div>
          </div>
        </div>
      {/each}
